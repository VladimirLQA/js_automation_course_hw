...
{
    log("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Task 1 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
    var testArrayOfNumbers = [1, 2, 3, 4, 5];
    var testArrayOfStrings = ['New', 'test', 'for', 'today'];
    var callBackNumber = function (el, i, array) { return log(el); };
    var callBackString = function (el, i, array) { return log(el); };
